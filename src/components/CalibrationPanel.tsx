      ...
      {loading ? (
        ...
        <div className="h-[220px] flex flex-col items-center justify-center text-xs text-muted-foreground gap-1">
          ...
          <span className="text-[10px]">
            Calibration appears after the engine accumulates outcomes.
          </span>
        </div>
      ) : (
        <>
            ...
            <div className="space-y-1">
              <div>
                ...
                <span className="ml-1 text-foreground">
                  Perfect calibration (confidence matches observed hit rate)
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Above diagonal:</span>
                <span className="ml-1 text-foreground">
                  Under-confident (actual success rate {">"} predicted)
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Below diagonal:</span>
                <span className="ml-1 text-foreground">
                  Over-confident (actual success rate {"<"} predicted)
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 mt-2 text-[10px]">
            ...
            <Stat
              label="Base rate"
              value={`${(data.baseRate * 100).toFixed(1)}%`}
              hint="actual hit %"
            />
          </div>
        </>
