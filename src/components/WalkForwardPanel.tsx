...
export function WalkForwardPanel({ coin }: Props) {
  ...
  useEffect(() => {
    setCost(DEFAULT_COSTS[coin.market]);
  }, [coin.market]);

  useEffect(() => {
    let cancelled = false;
    if (!coin.yahooSymbol) {
      setError("No historical symbol for this asset.");
      setResult(null);
      return;
    }
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        if (hist.length < 100) {
          setError("Not enough history for a walk-forward backtest.");
          setResult(null);
          setLoading(false);
          return;
        }
  ...
  return (
    <div className="panel p-4 space-y-3">
      <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">
            Expanding-window, no look-ahead. Trades sized to ±100% notional per signal · costs
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <NumField
          label="Fees (bps round-trip)"
          value={cost.feesBps}
          onChange={(v) => setCost((c) => ({ ...c, feesBps: v }))}
        />
        <NumField
          label="Slippage (bps round-trip)"
          value={cost.slippageBps}
          onChange={(v) => setCost((c) => ({ ...c, slippageBps: v }))}
        />
      </div>
      ...
      <div className="p-2.5 rounded border border-border/50 bg-card/50 space-y-1.5 text-[9px]">
        ...
        <div className="space-y-1">
          <div className="text-muted-foreground">
            Walk-forward testing simulates real trading: train on historical data, predict next N
            bars, compare prediction sign to actual return.
          </div>
          <div className="grid grid-cols-2 gap-2 text-[8px]">
            <div>
              <span className="font-bold text-foreground">✓ No Look-Ahead:</span> Each forecast uses
              only past data
            </div>
            <div>
              <span className="font-bold text-foreground">✓ Expanding Window:</span> Training set
              grows with each step
            </div>
            <div>
              <span className="font-bold text-foreground">✓ Cost-Aware:</span> Fees and slippage
              deducted
            </div>
            <div>
              <span className="font-bold text-foreground">✓ Sign-Based:</span> Only directional
              accuracy matters
            </div>
          </div>
        </div>
        ...
        <>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value, name) => {
                    ...
                    return [
                      Number.isFinite(v) ? `${((v - 1) * 100).toFixed(2)}%` : "—",
                      String(name),
                    ];
                  }}
                  contentStyle={{
          ...
          {m && (
            <div className="grid grid-cols-4 gap-2 mt-3 text-[10px]">
              <Stat
                label="Net return"
                value={`${(m.netReturn * 100).toFixed(2)}%`}
                good={m.netReturn > 0}
              />
              <Stat label="Trades" value={String(m.nTrades)} />
              <Stat
                label="Hit rate"
                value={`${(m.hitRate * 100).toFixed(1)}%`}
                good={m.hitRate > 0.5}
              />
              ...
              <Stat
                ...
                good={m.maxDrawdown < 0.1}
              />
              <Stat label="Calmar" value={m.calmar.toFixed(2)} good={m.calmar > 0.5} />
              <Stat
                label="Direction acc."
                value={`${(m.directionalAccuracy * 100).toFixed(1)}%`}
                good={m.directionalAccuracy > 0.52}
              />
            </div>
          )}
        </>
