...
export function hybridPredict(prices: number[], steps: number, options?: HybridOptions): HybridResult {
  ...
  const raw: number[] = [];
  let microWiggle = 0;
  // Keep a small volatility floor so the path does not become visually
  // straight in low-variance periods.
  const wiggleVolFloor = 0.10 * garch.sigma;
  const wiggleVol = Math.max(Math.abs(arima.residualStd), wiggleVolFloor);
  ...
  for (let i = 0; i < steps; i++) {
    ...
    microWiggle += gauss() * (wiggleVol * 0.12);
    const microCap = 0.9 * garch.sigma * sqrtT;
    microWiggle = Math.max(-microCap, Math.min(microCap, microWiggle));

    const price = last + trend + arimaWiggle + microWiggle;
    raw.push(price);
  }
